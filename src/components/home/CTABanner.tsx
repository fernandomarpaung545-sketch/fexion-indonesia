...
export function CTABanner() {
  return (
    <section style={{ background:'#0A2342', padding:'2rem 0', position:'relative', overflow:'hidden' }}>
      <div style={{ position:'absolute', inset:0, pointerEvents:'none',
        backgroundImage:'radial-gradient(ellipse 60% 100% at 50% 50%, rgba(37,99,235,0.1) 0%, transparent 70%)' }}/>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 1.5rem', position:'relative',
        ...
        <motion.div initial={{opacity:0,x:16}} whileInView={{opacity:1,x:0}} viewport={{once:true}}
          ...
          <Link href="/register" style={{
            ...
            padding:'0.75rem 1.625rem', borderRadius:8, textDecoration:'none',
            boxShadow:'0 4px 16px rgba(249,115,22,0.42)',
          }}><Users size={16}/> Join GEOFERA</Link>
          <Link href="/contact" style={{
            display:'inline-flex', alignItems:'center', gap:7,
            border:'1.5px solid rgba(255,255,255,0.22)', background:'rgba(255,255,255,0.06)',
            color:'#e2e8f0', fontWeight:600, fontSize:'0.9rem',
            padding:'0.75rem 1.625rem', borderRadius:8, textDecoration:'none',
          }}><Mic size={15}/> Become a Speaker</Link>
          <Link href="/contact" style={{
            display:'inline-flex', alignItems:'center', gap:7,
            border:'1.5px solid rgba(255,255,255,0.22)', background:'rgba(255,255,255,0.06)',
            color:'#e2e8f0', fontWeight:600, fontSize:'0.9rem',
            padding:'0.75rem 1.625rem', borderRadius:8, textDecoration:'none',
          }}><Handshake size={15}/> Become a Partner</Link>
        </motion.div>
      </div>
    </section>
  )
